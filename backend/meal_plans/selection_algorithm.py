...
# 早餐原本沒設上限，加上候選池夠大（≥12）時 MAX_RECIPE_REUSE 會降到 1（見上面），兩人 7 天
# 14 個早餐格位幾乎每份都不重複，選項變化量偏大；這裡給個上限，讓早餐可以在一週內重複出現。
CATEGORY_VARIETY_CAP = {"主食": 3, "肉": 3, "菜": 3, "下午茶": 2, "早餐": 5}
...
def select_recipe_for_slot(candidates: List[Dict], category: str, target_calories: float, target_protein: float,
    ...
    exclude_recipe_ids：「重新產生候選」時要排除的食譜（本次推薦流程裡這個類別已經展示過的），
    篩完是空池才退回原候選池（同樣的安全防呆，避免候選池太小時重推變成選不到菜）。
    ...
    if exclude_recipe_ids:
        filtered = [r for r in pool if r["id"] not in exclude_recipe_ids]
        if filtered:
            pool = filtered
...
def build_day_meals(candidates: List[Dict], user_a_ctx: Dict, user_b_ctx: Dict, meal_date,
                     ...
                     exclude_recipes_by_category: Optional[Dict[str, Set[int]]] = None,
                     recipe_scale_totals: Optional[Dict[int, float]] = None,
    ...
    exclude_recipes_by_category: {category: {recipe_id,...}}，見 select_recipe_for_slot 的
    exclude_recipe_ids，「重新產生候選」時排除本次推薦流程裡這個類別已經展示過的食譜。
    ...
    exclude_recipes_by_category = exclude_recipes_by_category or {}
        ...
        for meal_type, category in (("breakfast", "早餐"), ("afternoon_snack", "下午茶")):
            ...
            if preferred:
                ...
                recipe = select_recipe_for_slot(candidates, category, target_cal, target_protein,
                                                 ...
                                                 exclude_recipe_ids=exclude_recipes_by_category.get(category),
...
def generate_week_plan(candidates: List[Dict], user_a_ctx: Dict, user_b_ctx: Dict, week_start_date,
    ...
    exclude_recipes_by_category：見 build_day_meals，整週共用同一份排除清單。"""
