...
└── nutrition.db  # 搬遷前的 SQLite 舊資料，正式資料已在 Supabase Postgres
```
...
專案根目錄的 `nutrition.db` 是搬遷前的舊資料（見 `backend/migrate_to_postgres.py`），搬遷完成、確認 Postgres 資料無誤後可以封存或刪除，後端已不再讀取這個檔案。
...
- **前端**：部署到 GitHub Pages，由 `.github/workflows/deploy-pages.yml` 在 push 到 `main` 時自動建置部署，網址是 `https://<GitHub帳號>.github.io/Nutrition/`
- 前端建置時需要知道後端網址：在 repo 的 Settings → Secrets and variables → Actions → Variables 新增 `VITE_API_BASE_URL`，值是 Railway 後端網址加上 `/api`（例如 `https://xxx.up.railway.app/api`）

設定一次之後，之後改程式碼 push 到 GitHub，前後端都會自動重新部署。

### 把本機舊資料搬進去

`backend/migrate_to_postgres.py` 需要能連到 Postgres 的 5432/6543 埠才能跑，公司網路擋這兩個埠，所以這一步不能在公司網路的電腦上直接執行。兩個可行做法：

1. 換一個沒擋 port 的網路（例如手機熱點）跑一次 `python migrate_to_postgres.py`，跑完資料就在 Supabase 了，之後不用再搬
2. 或是等後端部署到 Railway 後，用 Railway 的 Shell 功能（Railway 後台 → 該服務 → Shell，等同在 Railway 的雲端環境裡開一個終端機）執行同一支腳本，因為是從 Railway 的網路連 Supabase，不會被公司網路擋到
